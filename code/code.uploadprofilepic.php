	...
	if(isset($_POST['submit-profile-pic'])){
		...
		if($fileActualExt == "jpg" || $fileActualExt == "jpeg"){
			...
			if($fileError === 0){
				...
				if($fileSize <= 500000){
					$fileNameNew = $_SESSION["user_name"].".".$fileActualExt;
					...
					if(move_uploaded_file($fileTmpName, $destinationUpload))
					{
						...
						header('Location: ../myaccount.php?istatus=uploadSuccess');
						exit();
					}else{
						header('Location: ../myaccount.php?istatus=uploadFail');
						exit();
					}
				}else{
					header('Location: ../myaccount.php?istatus=largefile');
					exit();
				}
			}else{
				header('Location: ../myaccount.php?istatus=uploadingError');
				exit();
			}
		}else{
			header('Location: ../myaccount.php?istatus=invalidExtension');
			exit();
		}
